 ...
 *    3. M77_FIFO_SIZE used in m77_echo
 *    4. Mixed usage of tab and space on the same source code
 * M: 1. Implemented m77Create and m77Close and pass them to iosDrvInstall.
...
|  PROTOTYPES                              |
+------------------------------------------*/
static int m77Create (M77_TY_CO_DEV *pChan, char *name, int mode);
static int m77Open (M77_TY_CO_DEV *pChan);
static int m77Close (M77_TY_CO_DEV *pChan);
...
)
{
    ...
    m77DrvNum[device_num] = iosDrvInstall (m77Create, (FUNCPTR) 0, m77Open,
                m77Close, tyRead, tyWrite, m77Ioctl);
...

{
    ...
    /* install the new isr */
    {
        ...
        if( devID != NULL ) {
            ...
            DBGWRT_1((DBH, "m77IrqInstall: Error calling vxbInstByNameFind, try with M77_intConnectRtn\n"));
